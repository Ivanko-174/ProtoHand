...
void setup() {
  ...
  Serial.print("Получено: ");  Serial.println(data.Pservo1);
...
void loop() {
  ...
  int sensorValue = analogRead(A4); // Считываем значение с потенциометра
  ...
  Serial.println(data.Pservo1);

  servo1.write(data.Pservo1);
  servo2.write(data.Pservo2);
  servo3.write(data.Pservo3);
  delay(10);
  }
  ...
  if (radio.available()) {
    radio.read(&data, sizeof(DataPacket));
    Serial.print("Получено: "); 
    Serial.println(data.Pservo1); // Посмотрите, меняется ли число при сгибе датчика
