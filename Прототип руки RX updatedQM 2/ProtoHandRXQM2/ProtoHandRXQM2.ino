#include <Arduino.h>
#include <Servo.h>

#include "Talkie.h"
#include "Vocab_US_Large.h"
#include "Vocab_US_Clock.h"
#include "Vocab_US_Acorn.h"

#include <SPI.h>
#include "nRF24L01.h"
#include "RF24.h"

#include <Arduino.h>
#include <Servo.h>
#include "Talkie.h"
#include "Vocab_US_Large.h"
#include "Vocab_US_Clock.h"
#include "Vocab_US_Acorn.h"
#include <SPI.h>
#include "nRF24L01.h"
#include "RF24.h"

Talkie voice;

Servo servo1, servo2, servo3, servo4, servo5;

RF24 radio(8, 10);
byte address[][6] = {"1Node", "2Node", "3Node", "4Node", "5Node", "6Node"};

struct DataPacket {
  uint8_t angles[5];
  bool speechCommand;
};

DataPacket packet;

// Сэмплы
const uint8_t sample1[]     PROGMEM = {};
const uint8_t sample2[]     PROGMEM = {};


// === Настройки режимов ===
const int modePin = 4;                    // Кнопка/переключатель режима
bool radioMode = true;                    // true = радио, false = локальные датчики

// Пины для локальных flex sensors
const int flexPins[5] = {A0, A1, A2, A3, A4};

//int currentAngles[5] = {90, 90, 90, 90, 90};

unsigned long lastSpeechTime = 0;
bool buttonPressed = false;

void setup() {
  Serial.begin(9600);
  pinMode(modePin, INPUT_PULLUP);
  pinMode(2, INPUT_PULLUP);        // кнопка речи

  // Радио
  radio.begin();
  radio.setAutoAck(1);
  radio.setRetries(0, 15);
  radio.setPayloadSize(32);
  radio.openReadingPipe(1, address[0]);
  radio.setChannel(0x60);
  radio.setPALevel(RF24_PA_MAX);
  radio.setDataRate(RF24_250KBPS);
  radio.powerUp();
  radio.startListening();

  servo1.attach(5);
  servo2.attach(6);
  servo3.attach(9);
  servo4.attach(10);
  servo5.attach(11);

  Serial.println("Приёмник запущен.");
  Serial.println("Нажмите кнопку на пине 4 для переключения режима");
}

void handleSpeechButton() {
  // custom created samples
  voice.say(sample1);
  voice.say(sample2);

  // VM61002 ROM (clock vocabulary, female US voice)
  voice.say(spc_GOOD);
  voice.say(spc_MORNING);
  delay(500);

  // Acorn computer speech synthesizer add on
  voice.say(spa_CASSETTE);
  voice.say(spa_DATA);
  voice.say(spa_ERROR);
  voice.say(spa_TONE1);
  voice.say(spa_TONE2);
  delay(500);

  delay(500);
  }

void loop() {
  // Переключение режима
  if (digitalRead(modePin) == LOW) {
    radioMode = !radioMode;
    Serial.print("Режим переключён: ");
    Serial.println(radioMode ? "РАДИО" : "ЛОКАЛЬНЫЕ ДАТЧИКИ");
    delay(300); // антидребезг
  }

  if (digitalRead(2) == LOW)
  {
    buttonPressed = true;
  }
  else {
    buttonPressed = false;
  }

  //exp
  radioMode = false;

  /*if (radioMode) {
    // === РЕЖИМ РАДИО ===
    if (radio.available()) {
      radio.read(&packet, sizeof(packet));
      moveServos(packet.angles);
    }
  } 
  //else {*/
    // === ЛОКАЛЬНЫЙ РЕЖИМ (датчики изгиба напрямую) ===
    /*uint8_t localAngles[5];
    for(int i = 0; i < 5; i++) {
      int val = analogRead(flexPins[i]);
      localAngles[i] = map(val, 300, 700, 0, 180);   
      localAngles[i] = constrain(localAngles[i], 0, 180);
    }
    moveServos(localAngles);*/
  //}
  int potent = analogRead(A0);
  potent = map(potent, 0, 1023, 0, 180);
  int potent1 = analogRead(A1);
  potent1 = map(potent1, 0, 1023, 0, 180);
  int potent2 = analogRead(A2);
  potent2 = map(potent2, 0, 1023, 0, 180);
  int potent3 = analogRead(A3);
  potent3 = map(potent3, 0, 1023, 0, 180);
  int potent4 = analogRead(A4);
  potent4 = map(potent4, 0, 1023, 0, 180);

  Serial.println(potent1);

  servo1.write(potent);
  servo2.write(potent1);
  servo3.write(potent2);
  servo4.write(potent3);
  servo5.write(potent4);

  bool buttonPressed = (digitalRead(2) == LOW);

  // Кнопка речи
  if (buttonPressed)
  {
      handleSpeechButton();
  }
  delay(20);
}

  

